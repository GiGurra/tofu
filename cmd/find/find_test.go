...
func TestExistsAccessibleDir(t *testing.T) {
	tmpDir := t.TempDir()

	// Test existing directory
	if !ExistsAccessibleDir(tmpDir) {
		t.Errorf("Expected ExistsAccessibleDir to return true for existing directory")
	}

	// Test non-existent directory
	if ExistsAccessibleDir(filepath.Join(tmpDir, "nonexistent")) {
		t.Errorf("Expected ExistsAccessibleDir to return false for non-existent directory")
	}
	...
	if ExistsAccessibleDir(file) {
		t.Errorf("Expected ExistsAccessibleDir to return false for file")
	}
}
