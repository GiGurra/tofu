...
Subcommands:
  ...
  validate  Validate a JWT token's signature and claims`,
	...
	cmd.AddCommand(jwtCreateCmd())
	cmd.AddCommand(jwtValidateCmd())
...
func jwtDecodeCmd() *cobra.Command {
	return boa.CmdT[JwtDecodeParams]{
		Use:   "decode [token]",
...
Displays the decoded Header, Payload (Claims), and the Signature.`,
		...
		RunFunc: func(params *JwtDecodeParams, cmd *cobra.Command, args []string) {
			token := params.Token
			if token == "" || token == "-" {
				// Read from stdin
				stat, _ := os.Stdin.Stat()
...
func jwtCreateCmd() *cobra.Command {
	return boa.CmdT[JwtCreateParams]{
		Use:   "create",
...
Examples:
  ...
  tofu jwt create -a none --subject "test" -e 1h`,
		ParamEnrich: defaultParamEnricher(),
		RunFunc: func(params *JwtCreateParams, cmd *cobra.Command, args []string) {
...
func jwtValidateCmd() *cobra.Command {
	return boa.CmdT[JwtValidateParams]{
		Use:   "validate [token]",
...
Examples:
  ...
  echo "eyJhbGci..." | tofu jwt validate -s "my-secret"`,
		ParamEnrich: defaultParamEnricher(),
		RunFunc: func(params *JwtValidateParams, cmd *cobra.Command, args []string) {
			token := params.Token
			if token == "" || token == "-" {
				// Read from stdin
				stat, _ := os.Stdin.Stat()
...
func runJwtCreate(params *JwtCreateParams, stdout io.Writer) error {
	// Validate algorithm
...
func runJwtValidate(params *JwtValidateParams, tokenString string, stdout io.Writer) error {
