...
func TestBase64Command(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input:    "hello world?",
			params:   Base64Params{UrlSafe: true},
			expected: "aGVsbG8gd29ybGQ_\n",
		},
		{
			name:     "URL Safe Decode",
			input:    "aGVsbG8gd29ybGQ_\n",
			params:   Base64Params{UrlSafe: true, Decode: true},
			expected: "hello world?",
		},
		{
			name:     "No Padding Encode",
			input:    "hello",
			params:   Base64Params{NoPadding: true},
			expected: "aGVsbG8\n",
		},
		{
			name:     "Custom Alphabet",
			input:    "hello",
			params:   Base64Params{Alphabet: "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_"},
			expected: "aGVsbG8=\n", // Same as std but with different chars if they differed
		},
		{
			name:    "Invalid Alphabet Length",
			input:   "hello",
			params:  Base64Params{Alphabet: "short"},
			wantErr: true,
		},
	}
