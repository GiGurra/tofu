...
func EnvCmd() *cobra.Command {
	return boa.CmdT[EnvParams]{
		...
		RunFunc: func(params *EnvParams, cmd *cobra.Command, args []string) {
...
func runEnv(params *EnvParams) error {
...
func listEnv(params *EnvParams) error {
	envVars := os.Environ()
...
func outputPlain(envMap map[string]string, keys []string, params *EnvParams) error {
...
func outputJSON(envMap map[string]string, keys []string, params *EnvParams) error {
...
func outputShell(envMap map[string]string, keys []string, params *EnvParams) error {
	for _, key := range keys {
		if params.Keys && !params.Values {
			fmt.Println(key)
		} else if params.Values && !params.Keys {
...
func outputPowershell(envMap map[string]string, keys []string, params *EnvParams) error {
	for _, key := range keys {
		if params.Keys && !params.Values {
			fmt.Println(key)
		} else if params.Values && !params.Keys {
